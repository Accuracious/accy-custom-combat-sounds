...
if math.random(1,5) == 1 then t:EmitSound("hurtmod_rev/hurt/" .. table.Random(sndHurt)) end

end

hook.Add("PlayerDeath","HS4PLYDie",function(victim,inflictor,attacker)
victim:EmitSound("hurtmod_rev/die/" .. table.Random(sndDie))
if inflictor:IsValid() && inflictor != victim then if math.random(1,5) == 1 then inflictor:EmitSound("hurtmod_rev/taunt/" .. table.Random(sndTaunt)) end end
end)
